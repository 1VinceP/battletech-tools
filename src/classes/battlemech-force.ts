...
export class BattleMechForce {
  ...
  public export(noInPlayVariabless: boolean = false): ICBTForceExport {
  ...
  getUnitViaUUID(uuid: string): BattleMech | null {
    for (let group of this.groups) {
      for (let unit of group.members) {
        if (unit.uuid === uuid) return unit;
      }
    }
    return null;
  }

  updateUnitViaUUID(nv: BattleMech): boolean {
  ...
  public getPhaseName(num: number): string {
    if (num === 0) {
