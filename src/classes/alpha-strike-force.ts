...
export default class AlphaStrikeForce {
  ...
  public getActiveMembers() {
    ...
    for (
      var memCountGroup = 0;
      memCountGroup < this.groups.length;
      memCountGroup++
    ) {
      for (var memCount = 0; memCount < this.groups.length; memCount++) {
        ...
        if (this.groups[memCountGroup].members[memCount].active)
          this.activeMembers++;
        this.forcePoints +=
          this.groups[memCountGroup].members[memCount].currentPoints / 1;
      }
    }

    this.membersLabel =
      " (" + this.activeMembers + "/" + this.groups.length + ")";
  }
  ...
  public addToGroup(mulUnit: AlphaStrikeUnit, groupIndex: number = 0) {
    ...
    if (this.groups.length > groupIndex && this.groups[groupIndex]) {
      ...
      this.groups[groupIndex].setAvailableFormationBonuses(
        formationBonuses.filter((x) => x.IsValid(this.groups[groupIndex])),
      );
      // }
    }
  }
  ...
  public moveUnitToGroup(
    ...
    if (
      ...
      if (this.groups[fromGroupIndex].members.length > fromUnitIndex) {
        ...
        this.groups[toGroupIndex].setAvailableFormationBonuses(
          formationBonuses.filter((x) => x.IsValid(this.groups[toGroupIndex])),
        );
        this.groups[fromGroupIndex].setAvailableFormationBonuses(
          formationBonuses.filter((x) =>
            x.IsValid(this.groups[fromGroupIndex]),
          ),
        );
      }
    }
  ...
  public removeUnitFromGroup(asGroupIndex: number, asUnitIndex: number) {
    if (this.groups.length > asGroupIndex && this.groups[asGroupIndex]) {
      if (
        this.groups[asGroupIndex].members.length > asUnitIndex &&
        this.groups[asGroupIndex].members[asUnitIndex]
      ) {
        this.groups[asGroupIndex].members.splice(asUnitIndex, 1);
        this.groups[asGroupIndex].setAvailableFormationBonuses(
          formationBonuses.filter((x) => x.IsValid(this.groups[asGroupIndex])),
        );
      }
    }
  }
  ...
  public export(noInPlayVariables: boolean = false): IASForceExport {
