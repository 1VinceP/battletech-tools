...
export class BattleMechGroup {
  ...
  public getName(indexNumber: number, forFavorites: boolean = false): string {
    if (this.customName && this.customName.trim()) {
      return this.customName;
    } else {
      if (forFavorites) return "Unnamed " + this.groupLabel;
      else return this.groupLabel + " #" + (indexNumber + 1).toString();
    }
  }
  ...
  public export(noInPlayVariabless: boolean = false): ICBTGroupExport {
