...
export default class AlphaStrikeGroup {
  ...
  public formationBonus?: IFormationBonus = formationBonuses.find(
    (x) => x.Name === "None",
  );
  ...
  constructor(importObj: IASGroupExport | null = null) {
    ...
    this.sortUnits();
    this.availableFormationBonuses = formationBonuses.filter((x) =>
      x.IsValid(this),
    );
  }
  ...
  public getActiveMembers() {
    ...
    for (var memCount = 0; memCount < this.members.length; memCount++) {
      ...
      if (this.members[memCount].active) this.activeMembers++;
      this.groupPoints += this.members[memCount].currentPoints / 1;
    }

    this.membersLabel =
      " (" + this.activeMembers + "/" + this.members.length + ")";
  }
  ...
  public setAvailableFormationBonuses(formationBonuses: IFormationBonus[]) {
    ...
    if (this.formationBonus) {
      ...
      if (
        !this.availableFormationBonuses.find((x) => x.Name === selectedBonus)
      ) {
  ...
  public export(noInPlayVariables: boolean = false): IASGroupExport {
  ...
  public import(importObj: IASGroupExport) {
    ...
    if (importObj.formationBonus) {
      this.formationBonus = formationBonuses.find(
        (x) => x.Name === importObj.formationBonus,
      );
    }
  }
}
