import {
  CONST_AS_PILOT_ABILITIES,
  IASPilotAbility,
} from "../data/alpha-strike-pilot-abilities";
import {
  CONST_AS_SPECIAL_ABILITIES,
  IASSpecialAbility,
} from "../data/alpha-strike-special-abilities";
...
export class AlphaStrikeUnit {
  ...
  public importMUL(incomingMechData: IASMULUnit) {
    if (typeof incomingMechData !== "undefined" && incomingMechData !== null) {
      ...
      if (incomingMechData.BFType) this.type = incomingMechData.BFType;
      ...
      while (tmpMove.indexOf('"') > 0) tmpMove = tmpMove.replace('"', "");
  ...
  public importUnit(incomingMechData: IAlphaStrikeUnitExport) {
    ...
    if (typeof incomingMechData !== "undefined" && incomingMechData !== null) {
      ...
      if (!this.damage.extreme) this.damage.extreme = 0;
      ...
      if (incomingMechData.pilot) this._pilot.import(incomingMechData.pilot);
  ...
  public get currentPilotAbility(): IASPilotAbility | null {
    if (this._pilot.alphaStrikeAbilities.length > 0) {
      for (let abi of CONST_AS_PILOT_ABILITIES) {
        if (this._pilot.alphaStrikeAbilities[0] === abi.id) return abi;
      }
    }
    return null;
  }
  ...
  public isUnderStrength(): boolean {
    ...
    if (
      ...
      for (
        let mpHitsCount = 0;
        mpHitsCount < this.vehicleMotive910.length;
        mpHitsCount++
      ) {
      ...
      for (
        let mpHitsCount = 0;
        mpHitsCount < this.vehicleMotive11.length;
        mpHitsCount++
      ) {
  ...
  public calcCurrentValues() {
    ...
    if (this.currentSkill < 4) {
      ...
      this.currentPoints =
        this.basePoints + pvDifference * (4 - this.currentSkill);
      ...
      this.currentPoints =
        this.basePoints - pvDifference * (this.currentSkill - 4);
    } else {
      // this.currentSkill = 4;
      this.currentPoints = this.basePoints;
    }
    ...
    if (
      typeof this.currentArmor === "undefined" ||
      this.currentArmor.length === 0
    ) {
      this.currentArmor = [];
    ...
    if (
      typeof this.currentStructure === "undefined" ||
      this.currentStructure.length === 0
    ) {
      this.currentStructure = [];
      for (
        let structureCount = 0;
        structureCount < this.structure;
        structureCount++
      )
    ...
    if (
      typeof this.engineHits === "undefined" ||
      this.engineHits.length === 0
    ) {
      this.engineHits = [];
    ...
    if (
      typeof this.fireControlHits === "undefined" ||
      this.fireControlHits.length === 0
    ) {
      this.fireControlHits = [];
    ...
    if (
      typeof this.vehicleMotive910 === "undefined" ||
      this.vehicleMotive910.length === 0
    ) {
      this.vehicleMotive11 = [];
    ...
    if (
      typeof this.vehicleMotive11 === "undefined" ||
      this.vehicleMotive11.length === 0
    ) {
      this.vehicleMotive11 = [];
      for (let hitCount = 0; hitCount < 2; hitCount++) {
    ...
    if (
      typeof this.mpControlHits === "undefined" ||
      this.mpControlHits.length === 0
    ) {
      this.mpControlHits = [];
      let numberOfHits = 4;
      if (
    ...
    if (
      typeof this.weaponHits === "undefined" ||
      this.weaponHits.length === 0
    ) {
      this.weaponHits = [];
    ...
    for (
      let weaponHitsCount = 0;
      weaponHitsCount < this.weaponHits.length;
      weaponHitsCount++
    ) {
      if (this.weaponHits[weaponHitsCount]) currentWeaponHits++;
    }

    let currentFCHits = 0;
    for (
      let fcHitsCount = 0;
      fcHitsCount < this.fireControlHits.length;
      fcHitsCount++
    ) {
      if (this.fireControlHits[fcHitsCount]) currentFCHits++;
    }
    ...
    for (
      let engineHitsCount = 0;
      engineHitsCount < this.engineHits.length;
      engineHitsCount++
    ) {
      if (this.engineHits[engineHitsCount]) currentEngineHits++;
    }
    ...
    if (shortDamage.toString().toString() !== "0*") {
      ...
      if (currentWeaponHits) shortDamage = 0;
    }

    if (mediumDamage.toString() !== "0*") {
      mediumDamage = +mediumDamage - currentWeaponHits;
    } else {
      if (currentWeaponHits) mediumDamage = 0;
    }

    if (longDamage.toString() !== "0*") {
      longDamage = +longDamage - currentWeaponHits;
    } else {
      if (currentWeaponHits) longDamage = 0;
    }

    if (extremeDamage.toString() !== "0*") {
      extremeDamage = +extremeDamage - currentWeaponHits;
    } else {
      if (currentWeaponHits) extremeDamage = 0;
    }

    if (+shortDamage < 0) shortDamage = 0;

    if (+mediumDamage < 0) mediumDamage = 0;

    if (+longDamage < 0) longDamage = 0;

    if (+extremeDamage < 0) extremeDamage = 0;
    ...
    for (let moveC = 0; moveC < this.move.length; moveC++) {
      ...
      if (this.move[moveC].type !== "j" || this.move.length === 1) {
        ...
        this.currentMoveHexesSprint =
          "" + Math.ceil((+this.move[moveC].currentMove / 2) * 1.5) + "⬣";
        // }
      ...
      this.currentMove +=
        "" + this.move[moveC].currentMove + '"' + this.move[moveC].type;
      this.currentMoveHexes +=
        "" + this.move[moveC].currentMove / 2 + "⬣" + this.move[moveC].type;

      // if( this.move[moveC].type === "j" ) {
      //     tmpTMM++;
      // }
      if (
        this.move[moveC].type === "j" &&
      ...
      if (this.move[moveC].currentMove === 0) tmpTMM = -4;
      ...
      if (this.move[moveC].currentMove > 0) this.immobile = false;
    ...
    this.currentToHitShort =
      this.currentSkill + this.currentHeat + currentFCHits * 2; // + currentEngineHits;
    this.currentToHitMedium =
      this.currentSkill + 2 + this.currentHeat + currentFCHits * 2; // + currentEngineHits;
    this.currentToHitLong =
      this.currentSkill + 4 + this.currentHeat + currentFCHits * 2; // + currentEngineHits;
    this.currentToHitExtreme =
      this.currentSkill + 6 + this.currentHeat + currentFCHits * 2; // + currentEngineHits;
    ...
    if (this.currentHeat > 4) this.currentHeat = 4;

    this.getCurrentStructure();

    if (currentEngineHits > 1) this.active = false;
  }
  /**
  ...
  public takeDamage(numberOfPoints: number) {
    ...
    for (let pointCounter = 0; pointCounter < numberOfPoints; pointCounter++) {
      for (
        let armorCounter = 0;
        armorCounter < this.currentArmor.length;
        armorCounter++
      ) {
      ...
      for (
        ...
        structureCounter < this.currentStructure.length;
        structureCounter++
      ) {
        if (this.currentStructure[structureCounter] === false) {
          if (leftOverPoints > 0) {
            ...
            if (this.getCurrentStructure() === 0) this.active = false;
  ...
  public getCurrentStructure() {
    ...
    if (structPoints < 1) this.active = false;
    else this.active = true;
  ...
  public export(noInPlayVariables: boolean = false): IAlphaStrikeUnitExport {
    // In Play Variables
  ...
  calc(): string {
    ...
    this.basePoints = 0;
    _calcLogAS +=
      '<div class="text-center"><strong> - Calculating Point Value - </strong></div>\n';
    /* *********************************
     * Step 1: Determine Unit’s Offensive Value ASC - p138
     * ******************************** */

    _calcLogAS +=
      "<strong>Step 1: Determine Unit’s Offensive Value ASC - p138</strong><br />\n";
     ...
     * ******************************** */
    _calcLogAS +=
      "<strong>Step 1a: Apply Blanket Offensive Modifiers ASC - p139</strong><br />\n";
    // TODO
     ...
     * ******************************** */
    _calcLogAS +=
      "<strong>Step 2: Determine Unit’s Defensive Value ASC - p139</strong><br />\n";
    ...
    if (_groundMove > this.jumpMove) {
      movementDefenseValue += _groundMove * 0.25;
      bestMovement = _groundMove;
    } else {
      movementDefenseValue += this.jumpMove * 0.25;
      bestMovement = _groundMove;
    }

    if (this.jumpMove > 0) {
      movementDefenseValue += 0.5;
      _calcLogAS +=
        "Movement Factor: " +
        movementDefenseValue +
        " ( " +
        bestMovement +
        " * .25 + .5)<br />\n";
    } else {
     ...
     * ******************************* */
    _calcLogAS +=
      "<strong>Step 2a: Calculating Defensive Interaction Rating (DIR) ASC - p141</strong><br />\n";
    let bmDIR = 0;
    // Armor Factor
    if (this.type.toLowerCase().trim() === "bm") {
      _calcLogAS +=
    ...
    if (defensiveFactor < 0) defensiveFactor = 0;
    bmDIR += defensiveFactor;
    _calcLogAS +=
      "Adding Defense Value from Step 2 above: " + defensiveFactor + "<br />\n";
    // Calculate the DIR
     ...
     * ******************************* */
    _calcLogAS +=
      "<strong>Step 3: Determine Unit’s Final Point Value ASC - p141</strong><br />\n";
    ...
    if (
      ...
      _calcLogAS +=
        'Unit has 6 to 10" of Move, but only delivers damage at Short range. Point Value * .75<br />\n';
      _calcLogAS +=
        "Modified Point Value: " +
        baseFinalValue * 0.75 +
        " ( " +
        offensive_value +
        " + " +
        bmDIR +
        " )<br />\n";
      finalValue = baseFinalValue * 0.75;
    }
    ...
    if (
      ...
      _calcLogAS +=
        'Unit has 2 to 5" of Move, but only delivers damage at Short range. Point Value * .5<br />\n';
      _calcLogAS +=
        "Modified Point Value: " +
        baseFinalValue * 0.5 +
        " ( " +
    ...
    if (
      ...
      _calcLogAS +=
        'Unit has 2 to 5" of Move, but only delivers damage at Short and Medium ranges. Point Value * .75<br />\n';
      _calcLogAS +=
        "Modified Point Value: " +
        baseFinalValue * 0.75 +
        " ( " +
     ...
     * ******************************* */
    _calcLogAS +=
      "<strong>Step 3a: Add Force Bonuses ASC - p141</strong><br />\n";
    // TODO
