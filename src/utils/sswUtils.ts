export function getSSWRulesLevelLabel(
  ...
  if (num === 0) {
    ...
    else return "Introductory";
  }
  if (num === 1) {
    if (short) return "Std";
    else return "Tournament Legal (Standard)";
  }
  if (num === 2) {
    if (short) return "Adv";
  ...
  if (num === 4) {
    if (short) return "Era";
    else return "Era Specific";
  }

  return "n/a";
}

export function isSSWRulesLevel(num: number, label: string): boolean {
  ...
  if (
    num === 1 &&
    getSSWRulesLevelLabel(1).toLowerCase().trim().indexOf(label) > -1
  ) {
    return true;
  }
