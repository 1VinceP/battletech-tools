...
export function getSSWXMLBasicInfo(ssw_xml: string): ISSWBasicInfo | null {
  const options = {
  ...
  if (jObj && jObj.mech) {
    ...
    if (
      jObj.mech.baseloadout &&
      jObj.mech.baseloadout.battleforce &&
      jObj.mech.baseloadout.battleforce["@_pv"]
    ) {
