...
class FormationBonusBase {
  CheckIdealRole(group: AlphaStrikeGroup, role: string): boolean {
    for (let i = 0; i < group.members.length; i++) {
      if (group.members[i].role !== role) return false;
    }
    return true;
  }
...
class BattleLance extends FormationBonusBase implements IFormationBonus {
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.5) <=
        group.members.filter((x) => x.size >= 3).length;
    let brawlerSniperSkirmisherCount =
      this.RoleCount(group, "Brawler") +
      this.RoleCount(group, "Sniper") +
      this.RoleCount(group, "Skirmisher");
...
class LightBattleLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "The Battle Lance formation receives the equivalent of a Lucky Special Pilot Ability (see p. 97) as a level of the number of units in the formation at Setup plus two. So a Lance of 4 ‘Mechs in a Battle Lance receives a 6-point Lucky Special Ability. It is useable by any unit in the Battle Lance, rather than limited to any single unit. This bonus ability may be stacked with a Lucky SPA assigned to one or more of the Battle Lance’s member units. But if this is done, the maximum number of rerolls a Battle Lance unit may attempt for the duration of the scenario— between both the pilot’s ability and that provided bythis formation bonus—is 4.";
  RequirementsDescription: string =
    "At least 75 percent of this Lance must be Size 1, and there may be no units of Size 4 of higher in this formation type. If this is a vehicle formation, there must be at least 2 matched pairs of Size 1 units. At least one of the units in a Light Battle Lance must be of the Scout unit role.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.75) <=
        group.members.filter((x) => x.size === 1).length;
...
class MediumBattleLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "The Battle Lance formation receives the equivalent of a Lucky Special Pilot Ability (see p. 97) as a level of the number of units in the formation at Setup plus two. So a Lance of 4 ‘Mechs in a Battle Lance receives a 6-point Lucky Special Ability. It is useable by any unit in the Battle Lance, rather than limited to any single unit. This bonus ability may be stacked with a Lucky SPA assigned to one or more of the Battle Lance’s member units. But if this is done, the maximum number of rerolls a Battle Lance unit may attempt for the duration of the scenario— between both the pilot’s ability and that provided bythis formation bonus—is 4.";
  RequirementsDescription: string =
    "At least 50 percent of the Medium Battle Lance must be of Size 2, and there may be no units of Size 4 or larger in this formation at all. If this is a vehicle formation, there must be at least 2 matched pairs of Size 2 units.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.5) <=
        group.members.filter((x) => x.size === 2).length;
    return result;
  }
}
class HeavyBattleLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "The Battle Lance formation receives the equivalent of a Lucky Special Pilot Ability (see p. 97) as a level of the number of units in the formation at Setup plus two. So a Lance of 4 ‘Mechs in a Battle Lance receives a 6-point Lucky Special Ability. It is useable by any unit in the Battle Lance, rather than limited to any single unit. This bonus ability may be stacked with a Lucky SPA assigned to one or more of the Battle Lance’s member units. But if this is done, the maximum number of rerolls a Battle Lance unit may attempt for the duration of the scenario— between both the pilot’s ability and that provided bythis formation bonus—is 4.";
  RequirementsDescription: string =
    "At least 50 percent of the Heavy Battle Lance must be of Size 3 or higher, and there may be no Size 1 units in this formation. If this is a vehicle formation, there must be at least 2 matched pairs of Size 3 units.";
...
class AssaultLance extends FormationBonusBase implements IFormationBonus {
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    let medRange3OrGreaterCount = group.members.filter(
      (x) => +x.damage.medium >= 3,
    ).length;
    result =
      result &&
      Math.ceil(group.members.length * 0.75) <= medRange3OrGreaterCount;
    result =
      result &&
      (this.RoleCount(group, "Juggernaut") >= 1 ||
        this.RoleCount(group, "Sniper") >= 2);
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      ...
      group.members.filter(
        (x) =>
          x.move.filter((y) => y.type === "j").length > 0 ||
          x.move.filter((y) => y.move >= 10).length > 0,
      ).length === group.members.length;

    return result;
  }
}
class StrikerCavalryLance
  extends FormationBonusBase
  implements IFormationBonus
{
  ...
  BonusDescription: string =
    "75 percent of the units in a standard Striker/ Cavalry Lance receive the Speed Demon Special Pilot Ability (see p. 99).";
  RequirementsDescription: string =
    "All units in a Striker/Cavalry Lance must have a minimum ground Move of 10” or a jumping Move of 8”j. No units in a Striker/Cavalry Lance may be of Size 4 or above. At least 50 percent of the Striker/Cavalry Lance must be of the Striker or Skirmisher unit role";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.5) <=
        this.RoleCount(group, "Striker") + this.RoleCount(group, "Skirmisher");
    ...
    result =
      ...
      group.members.filter(
        (x) =>
          x.move.filter((y) => y.type === "j" && y.move >= 8).length > 0 ||
          x.move.filter((y) => y.move >= 10 && y.type !== "j").length > 0,
      ).length > 0;

    return result;
  }
}
class LightStrikerCavalryLance
  extends FormationBonusBase
  implements IFormationBonus
{
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result && group.members.filter((x) => +x.damage.long > 0).length >= 2;
    ...
    result =
      result &&
      group.members.filter((x) => x.move.filter((y) => y.move >= 10).length > 0)
        .length === group.members.length;

    return result;
  }
}
class HeavyStrikerCavalryLance
  extends FormationBonusBase
  implements IFormationBonus
{
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    //must have 2 with long damage
    result =
      result && group.members.filter((x) => +x.damage.long > 1).length >= 1;
    ...
    result =
      result &&
      group.members.filter((x) => x.move.filter((y) => y.move >= 8).length > 0)
        .length === group.members.length;
...
class FireLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of each turn, up to half the Fire Lance units (rounded down) may receive the Sniper Special Pilot Ability (see p. 99), which will affect their weapon attacks during that turn. Destroyed or withdrawn units do not count towards the current number of units in the formation.";
  RequirementsDescription: string =
    "At least 75 percent of the units in a standard Fire Lance must be of either the Missile Boat or Sniper unit roles.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.75) <=
        this.RoleCount(group, "Missile Boat") + this.RoleCount(group, "Sniper");
    return result;
  }
}
class FireSupportLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of each turn, up to half the Fire Support Lance units (rounded down) may receive the Oblique Attacker Special Pilot Ability (see p. 98), which will affect their weapon attacks during that turn. Destroyed or withdrawn units do not count towards the current number of units in the formation.";
  RequirementsDescription: string =
    "To serve as a Fire Support Lance, at least 3 units in this formation must possess the Indirect Fire (IF#) special ability.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      group.members.filter((x) => x.abilities.includes("IF")).length >= 3;
    return result;
  }
}
class ArtilleryFireLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of each turn, up to half the Artillery Fire Lance units (rounded down) may receive the Oblique Artilleryman Special Pilot Ability (see p. 98), which will affect their weapon attacks during that turn. Destroyed or withdrawn units do not count towards the current number of units in the formation.";
  RequirementsDescription: string =
    "To serve as an Artillery Fire Lance, at least 2 units in this formation must have an Artillery (ARTX-#) special ability.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      group.members.filter((x) => x.abilities.includes("ART")).length >= 2;
    return result;
  }
}
...
class DirectFireLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of each turn, up to half the Direct Fire Lance units (rounded down) may receive the Weapon Specialist Special Pilot Ability (see p. 101), which will affect their weapon attacks during that turn. Destroyed or withdrawn units do not count towards the current number of units in the formation.";
  RequirementsDescription: string =
    "At least 2 units in a Direct Fire Lance must be of Size 3 or larger, and all units in this formation must be able to deliver at least 2 points of damage to their Long-range attack bracket.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result && group.members.filter((x) => +x.damage.long < 2).length === 0;
    return result;
  }
}
class AntiAirLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of each turn, up to half the Anti-Air Lance units (rounded down) may receive the effects of the Anti-Aircraft Specialists Special Command Ability (see p. 102), which will affect their weapon attacks during that turn. Destroyed or withdrawn units do not count towards the current number of units in the formation.";
  RequirementsDescription: string =
    "In addition to the requirements established for the standard Fire Lance, at least 2 units in an Anti-Air Lance must possess the Flak (FLK#), Autocannon (AC#/#/#), or Artillery (ARTX-#) special abilities.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      group.members.filter((x) => x.abilities.includes("FLK")).length +
        group.members.filter((x) => x.abilities.includes("AC")).length +
        group.members.filter((x) => x.abilities.includes("ART")).length >=
        2;
    return result;
  }
}
...
class ReconLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of play, the Recon Lance’s controlling player must choose either the Eagle’s Eyes, Forward Observer, or Maneuvering Ace SPAs (see pp. 95, 96, and 97, respectively). Every unit in this Recon Lance receives the chosen SPA. Note that the abilities chosen at the scenario’s start cannot be exchanged for a different ability during the course of the same scenario.";
  RequirementsDescription: string =
    "All units in a Recon Lance must possess a minimum Move of 10”. At least 2 units in this formation type must also be of the Scout or Striker unit roles.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      this.RoleCount(group, "Scout") + this.RoleCount(group, "Striker") >= 2;
    result =
      result &&
      group.members.filter((x) => x.move.filter((y) => y.move >= 10).length > 0)
        .length === group.members.length;
...
class LightRecon extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of play, the Light Recon Lance’s controlling player can choose from the Eagle’s Eyes, Forward Observer, or Maneuvering Ace SPAs (see pp. 95, 96, and 97, respectively) for each unit. Note that the abilities chosen at the scenario’s start cannot be exchanged for a different ability during the course of the same scenario.";
  RequirementsDescription: string =
    "All units in a Light Recon Lance must be of Size 1, with a minimum Move of 12” (with or without jump capability). Furthermore, all of these units must be of the Scout unit role.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      group.members.filter((x) => x.size === 1).length === group.members.length;
    result =
      result &&
      group.members.filter((x) => x.move.filter((y) => y.move >= 12).length > 0)
        .length === group.members.length;
...
class HeavyReconLance extends FormationBonusBase implements IFormationBonus {
  ...
  BonusDescription: string =
    "At the beginning of play, the Heavy Recon Lance’s controlling player must choose either the Eagle’s Eyes, Forward Observer, or Maneuvering Ace SPAs (see pp. 95, 96, and 97, respectively). Half the units in this Heavy Recon Lance receives the chosen SPA. Note that the abilities chosen at the scenario’s start cannot be exchanged for a different ability during the course of the same scenario.";
  RequirementsDescription: string =
    "All units in a Heavy Recon Lance must have a Move of 8” of more, with no less than 2 able to move 10” or more (all with or without jump capability). At least 1 unit in this formation type must be of Size 3 or larger. Finally, at least 2 units in a Heavy Recon Lance must be of the Scout unit role.";
...
class PursuitLance extends FormationBonusBase implements IFormationBonus {
  ...
  RequirementsDescription: string =
    "All units in a Pursuit Lance must be of Size 2 or less, and 75 percent of this formation must have a Move of 12” or more, regardless of jumping capability. At least 1 unit in the Pursuit Lance must have a Medium-range attack value over 1 point.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      group.members.filter((x) => x.size <= 2).length === group.members.length;
    result =
      result &&
      Math.ceil(group.members.length * 0.75) <=
        group.members.filter(
          (x) => x.move.filter((y) => y.move >= 12).length > 0,
        ).length;
    result =
      result && group.members.filter((x) => +x.damage.medium <= 1).length >= 1;
    return result;
  }
}
class ProbeLance extends FormationBonusBase implements IFormationBonus {
  ...
  RequirementsDescription: string =
    "All units in a Probe Lance must be of Size 3 or less, and 75 percent must have a Move of 10” or more, with or without jump capability. All Probe Lance units must be able to deliver at least 2 points of damage at Medium range.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      ...
      Math.ceil(group.members.length * 0.75) <=
        group.members.filter(
          (x) => x.move.filter((y) => y.move >= 10).length > 0,
        ).length;
    result =
      result && group.members.filter((x) => +x.damage.medium < 2).length === 0;
...
class SweepLance extends FormationBonusBase implements IFormationBonus {
  ...
  RequirementsDescription: string =
    "All units in a Sweep Lance must be of Size 2 or less, and have a Move of 10” or more, regardless of jumping capability. All Sweep Lance units must be able to deliver at least 2 points of damage at Short range.";
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      ...
      group.members.filter((x) => x.move.filter((y) => y.move >= 10).length > 0)
        .length === group.members.length;
    result =
      result && group.members.filter((x) => +x.damage.short < 2).length === 0;
...
class CommandLance extends FormationBonusBase implements IFormationBonus {
  ...
  IsValid(group: AlphaStrikeGroup): boolean {
    ...
    result =
      result &&
      Math.ceil(group.members.length * 0.5) <=
        this.RoleCount(group, "Missile Boat") +
          this.RoleCount(group, "Sniper") +
          this.RoleCount(group, "Skirmisher") +
          this.RoleCount(group, "Juggernaut");
    result =
      result &&
      this.RoleCount(group, "Brawler") +
        this.RoleCount(group, "Striker") +
        this.RoleCount(group, "Scout") >=
        1;
