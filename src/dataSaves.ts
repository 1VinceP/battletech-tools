...
export async function getFullBackup(appSettings: AppSettings): Promise<string> {
  let rv: IFullBackup = {
...
export function checkFullRestoreData(io: IFullBackup): boolean {
    ...
    for (let item of io.favoriteASGroups) {
      ...
      for (let existingItem of appGlobals.favoriteASGroups) {
        ...
        if (existingItem.uuid === item.uuid) {
          ...
          restoreMessages.push({
            severity: "replace",
            message:
              "Replace Alpha Strike Favorite Group '" +
              existingName +
              "' with '" +
              itemName +
              "'",
      ...
      if (!foundItem) {
        restoreMessages.push({
          severity: "add",
          message:
            "Add To your Alpha Strike Favorite groups: '" + itemName + "'",
        });
        if (performActions) {
    ...
    for (let item of io.favoriteCBTGroups) {
      ...
      for (let existingItem of appGlobals.favoriteCBTGroups) {
        ...
        if (existingItem.uuid === item.uuid) {
          ...
          restoreMessages.push({
            severity: "replace",
            message:
              "Replace Classic BattleTech Favorite Group '" +
              existingName +
              "' with '" +
              itemName +
              "'",
      ...
      if (!foundItem) {
        restoreMessages.push({
          severity: "add",
          message:
            "Add To your Classic BattleTech Favorite groups: '" +
            itemName +
            "'",
        });
        if (performActions) {
...
export async function getBattleMechSaves(
  ...
  try {
    if (rawData) rv = JSON.parse(rawData);
...
export async function getCurrentBattleMech(
  ...
  return await getData(appSettings, "currentBattleMech");
}
...
export function saveAppSettings(newValue: IAppSettingsExport) {
