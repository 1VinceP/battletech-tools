...
> {
  ...
  updateText = (event: React.FormEvent<HTMLInputElement>) => {
    if (this.props.setValue) this.props.setValue(event.currentTarget.value);
  ...
  setText = (newValue: string) => {
    ...
    if (this.props.setValue) this.props.setValue(newValue);
    if (this.props.onChange)
      this.props.onChange(event as React.FormEvent<HTMLInputElement>);

    if (this.props.onSetCharacter) this.props.onSetCharacter();
  ...
  render = (): JSX.Element => {
    ...
    let label = "";
    if (this.props.label && this.props.label.trim()) label = this.props.label;

    let type = "text";
    if (this.props.type) type = this.props.type;

    let fieldReadOnly = false;
    if (this.props.readOnly) fieldReadOnly = true;
      ...
      <div className="position-relative">
        ...
        {this.props.ddList && this.props.ddList.length > 0 ? (
          <>
            ...
            >
              {this.state.ddOpen ? <FaChevronDown /> : <FaChevronUp />}
            </button>
            {this.state.ddOpen ? (
              <ul className="dd-list">
                {this.props.ddList.map((item, itemIndex) => {
                  return (
                    <li key={itemIndex} onClick={() => this.setText(item)}>
                      {item}
                    </li>
                  );
                })}
        ...
        >
          ...
          <input
            ...
            value={value}
            className={
              this.props.inline ? "width-auto inline-block" : "full-width"
            }
