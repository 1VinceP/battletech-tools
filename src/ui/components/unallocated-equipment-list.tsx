...
export default class UnallocatedEquipmentList extends React.Component<
  IUnallocatedEquipmentListProps,
  IUnallocatedEquipmentListState
> {
  render = (): JSX.Element => {
    return (
      <ul className="critical-assignment-container">
        {this.props.crits.map((crit, critIndex) => {
          if (crit) {
            let selectedClass = "";
            if (
              this.props.currentSelectedLocation === "un" &&
            ...
            return (
              <li
                key={critIndex}
                className={
                  "cursor-pointer critical-height-" + crit.crits + selectedClass
                }
                onClick={() =>
