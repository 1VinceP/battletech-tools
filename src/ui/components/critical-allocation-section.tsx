...
export default class CriticalAllocationSection extends React.Component<
  ICriticalAllocationSectionProps,
  ICriticalAllocationSectionState
> {
  ...
  render = (): JSX.Element => {
    ...
    for (let critIndex = 0; critIndex < this.props.crits.length; critIndex++) {
      ...
      if (
        ...
        this.state.hoveredIndex + this.props.currentSelectedItemSize - 1 >=
          critIndex &&
        ...
        if (
          this.props.mech.criticalCanBePlaced(
        ...
        else clickableClass = " cannot-be-placed ";
      ...
      if (crit) {
        if (!crit.placeholder) {
          ...
          critItems.push(
            <li className={"critical-placeholder critical-height-"}></li>,
          );
        }
      } else {
        critItems.push(
    ...
    return (
      ...
      >
        {critItems.map((critJSXItem, critIndex) => {
          return <React.Fragment key={critIndex}>{critJSXItem}</React.Fragment>;
        })}
      </ul>
    );
  };
