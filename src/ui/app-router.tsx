...
import AlphaStrikeForce, {
  IASForceExport,
} from "../classes/alpha-strike-force";
import AlphaStrikeGroup, {
  IASGroupExport,
} from "../classes/alpha-strike-group";
...
import {
  ...
  getBattleMechSaves,
  getCurrentASForce,
  getCurrentBattleMech,
  getCurrentCBTForce,
  getFavoriteASGroups,
  getFavoriteCBTGroups,
  saveAppSettings,
  saveBattleMechSaves,
  saveCurrentASForce,
  saveCurrentBattleMech,
  saveCurrentCBTForce,
  saveFavoriteASGroups,
  saveFavoriteASGroupsObjects,
  saveFavoriteCBTGroupsObjects,
} from "../dataSaves";
...
*/
  ...
  setData = async (appSettings: AppSettings, appGlobals: IAppGlobals) => {
    let asImport: IASForceExport | null = await getCurrentASForce(appSettings);
    ...
    let battleMechSaves: IBattleMechExport[] =
      await getBattleMechSaves(appSettings);
    let asImportFavorites: IASGroupExport[] =
      await getFavoriteASGroups(appSettings);
    ...
    if (needsBMReSave) {
      console.info(
        "Some UUIDs not found in BattleMech saves, generating UUIDS and re-saving...",
      );
    ...
    if (needsASReSave) {
      console.info(
        "Some UUIDs not found in AlphaStrike Favorites saves, generating UUIDS and re-saving...",
      );
    ...
    let bmImportFavorites: ICBTGroupExport[] =
      await getFavoriteCBTGroups(appSettings);
    ...
    let bmfImport: ICBTForceExport | null =
      await getCurrentCBTForce(appSettings);
  ...
  saveASGroupFavorite = (asGroup: AlphaStrikeGroup): void => {
    ...
    saveFavoriteASGroupsObjects(
      appGlobals.appSettings,
      appGlobals.favoriteASGroups,
    );
  };
  ...
  makeDocumentTitle = (subTitle: string = ""): void => {
    ...
    callAnalytics(window, appGlobals.sessionUUID, pjson.version);
  ...
  render = (): JSX.Element => {
    return (
      <>
        <Modal
          show={this.state.appGlobals.showConfirmDialog}
        ...
        >
          ...
          <Modal.Body>
            <div className="form">
              <div>
                <SanitizedHTML
                  html={this.state.appGlobals.confirmDialogMessage}
                />
              </div>
            </div>
          </Modal.Body>
          <Modal.Footer>
            ...
            <button
              ...
              onClick={this.confirmConfirmDialog}
            >
        ...
        <Router>
          <Routes>
            ...
            <Route
              path={`${process.env.PUBLIC_URL}/ssw-sanity-check`}
              element={<SSWSanityCheck appGlobals={this.state.appGlobals} />}
            />

            <Route
              path={`${process.env.PUBLIC_URL}/dev-status`}
              element={<DevelopmentStatus appGlobals={this.state.appGlobals} />}
            />
            <Route
              path={`${process.env.PUBLIC_URL}/about`}
              element={<About appGlobals={this.state.appGlobals} />}
            />
            <Route
              path={`${process.env.PUBLIC_URL}/settings/*`}
              element={<SettingsRouter appGlobals={this.state.appGlobals} />}
            />
            <Route
              path={`${process.env.PUBLIC_URL}/alpha-strike-roster/*`}
              element={
                <Navigate
                  to={`${process.env.PUBLIC_URL}/alpha-strike/roster/`}
                />
              }
            />

            <Route
              path={`${process.env.PUBLIC_URL}/mech-creator/*`}
              element={
                <Navigate
            ...
            <Route
              path={`${process.env.PUBLIC_URL}/alpha-strike/*`}
              element={<AlphaStrikeRouter appGlobals={this.state.appGlobals} />}
            />
            <Route
              path={`${process.env.PUBLIC_URL}/classic-battletech/*`}
              element={
                <ClassicBattleTechRouter appGlobals={this.state.appGlobals} />
              }
            />

            <Route
              path="*"
              element={<Error404 appGlobals={this.state.appGlobals} />}
            />
          </Routes>
        </Router>
      </>
