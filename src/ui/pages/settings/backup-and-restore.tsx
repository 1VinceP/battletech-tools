...
export default class SettingsBackupAndRestore extends React.Component<
  ISettingsBackupAndRestoreProps,
  ISettingsBackupAndRestoreState
> {
  ...
  getFullBackupString = async () => {
    let fullBackupString = await getFullBackup(
  ...
  doRestore = (e: React.FormEvent<HTMLButtonElement>) => {
  ...
  updateOverwriteCurrentBatthemech = (e: React.FormEvent<HTMLInputElement>) => {
    this.setState({
      overwriteCurrentBattlemech: e.currentTarget.checked,
    });
  };

  updateOverwriteCurrentASGroup = (e: React.FormEvent<HTMLInputElement>) => {
    this.setState({
      overwriteCurrentASGroup: e.currentTarget.checked,
    });
  };

  updateOverwriteCurrentCBTGroup = (e: React.FormEvent<HTMLInputElement>) => {
    this.setState({
        ...
        if (content) {
          ...
          if (checkFullRestoreData(data)) {
            ...
            this.setState({
              fullRestoreObject: null,
              restoreError:
                "Sorry, this does not appear to be a valid restore file",
              overwriteCurrentBattlemech: false,
              overwriteCurrentASGroup: false,
              overwriteCurrentCBTGroup: false,
            });
  ...
  render = (): JSX.Element => {
    let restoreFileName =
      this.restoreFileName + "-" + new Date().toLocaleString() + ".json";
    return (
      <UIPage
        current="settings-backup-and-restore"
        ...
        <div className="row">
          <div className="col-md-6">
            <fieldset className="fieldset">
              ...
              <p>
                Getting OAUth Sycning on a github static page URL has been
                difficult, so here's a workaround: Saving and Restoring all Data
                Manually
              </p>
              ...
              <div className="text-center">
                ...
                <p>
                  Clicking on the above button will download{" "}
                  <strong>ALL</strong> data which you can store on your own
                  devices or cloud services to restore to a new device
                </p>
              </div>
              <hr />
              ...
              {this.state.fullRestoreObject ? (
                <>
                  <h4>
                    Huzzah! We're able to restore from the data on this file!
                  </h4>
                ...
                <>
                  <p>
                    Restore from Backup. The file is named on export as "
                    {this.restoreFileName}-(date string).json", but you may have
                    renamed it.
                  </p>
                  ...
                  <ul>
                    ...
                    <li>
                      Overwrite App SettingsBackupAndRestore, including theme,
                      search history, etc
                    </li>
                    <li>
                      We'll ask you to overwrite your current Alpha Strike
                      Roster, defaulting to "no"
                    </li>
                    <li>
                      Merge your Saved Alpha Strike Groups
                      <br />
                      <div className="small-text">
                        Each group has a unique UUID which we'll use to
                        overwrite if you've renamed it, any non-existing UUIDs
                        Groups will be appended to your current Saved Groups
                      </div>
                    </li>
                    <li>
                      We'll ask you to overwrite your currently editing
                      BattleMech, defaulting to "no"
                    </li>
                    <li>
                      Merge your Saved BattleMechs
                      <br />
                      <div className="small-text">
                        Each 'mech has a unique UUID which we'll use to
                        overwrite if you've renamed it, any non-existing UUIDs
                        Groups will be appended to your current saved 'mech list
                      </div>
                    </li>
                  </ul>
                  ...
                  <label title="Click here to select a JSON file exported by the button to the right">
                    Restore Settings:&nbsp;
