...
export default class AlphaStrikeUnitEditViewModal extends React.Component<
  IAlphaStrikeUnitEditViewModalProps,
  IAlphaStrikeUnitEditViewModalState
> {
      ...
      <StandardModal
        ...
        title={
          this.props.showASUnit
            ? "Showing Unit: " +
              (this.props.showASUnit ? this.props.showASUnit.name : "")
            : "Editing Unit:" +
              (this.props.showASUnit ? this.props.showASUnit : "")
        }
      >
              ...
              <div className="row">
                <div className="col-xs-12 col-lg-12 text-left">
                      ...
                      <label>
                        ...
                        >
                          ...
                          {CONST_AS_PILOT_ABILITIES.map((abi, abiIndex) => {
                            return (
                              <option
                                title={abi.summary.join("\n")}
                                key={abiIndex}
                                value={abi.id}
                              >
                                {abi.ability} ({abi.cost})
                              </option>
                            );
                          })}
                        </select>
                        {this.props.showASUnit.currentPilotAbility ? (
                          <div>
                            {this.props.showASUnit.currentPilotAbility.summary}
                          </div>
                        ) : null}
                      </label>
                      ...
                      {this.props.showASUnit.currentSkill <= 3 ? (
                        <>
                          <label>
                            ...
                            <select
                              value={
                                this.props.showASUnit.currentPilotAbilityID2
                              }
                          ...
                          {this.props.showASUnit.currentSkill <= 1 ? (
                            <>
                              <label>
                                ...
                                <select
                                  value={
                                    this.props.showASUnit.currentPilotAbilityID3
                                  }
                                  onChange={this.updateUnitPilotAbility3}
                                >
                                  <option value={0}>- None -</option>
                                  {CONST_AS_PILOT_ABILITIES.map(
                                    (abi, abiIndex) => {
                                      return (
                                        <option
                                          title={abi.summary.join("\n")}
                  ...
                  <div className="text-right">
                    <strong>
                      Total Pilot Card Points:{" "}
                      {this.props.showASUnit.getTotalPilotAbilityPoints()}
                    </strong>
                  </div>
                  <br />
                </div>
