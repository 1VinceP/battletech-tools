...
export default class CurrentForceList extends React.Component<
  ICurrentForceListProps,
  ICurrentForceListState
> {
  ...
  updateFormationBonus = (
    ...
    if (this.props.appGlobals.currentASForce) {
      this.props.appGlobals.currentASForce.groups[groupIndex].formationBonus =
        formationBonuses.find((x) => x.Name === event.currentTarget.value);
  ...
  toggleContextMenuForce = (
    ...
    if (
      this.state.contextMenuGroup !== asGroupIndex &&
      this.state.contextMenuUnit !== asUnitIndex
    ) {
  ...
  removeGroup = (groupIndex: number): void => {
    if (
      this.props.appGlobals.currentASForce &&
      this.props.appGlobals.currentASForce.groups.length > groupIndex
    ) {
      if (
  ...
  moveUnitToGroup = (
    ...
    if (this.props.appGlobals.currentASForce) {
      this.props.appGlobals.currentASForce.moveUnitToGroup(
        fromUnitIndex,
        fromGroupIndex,
        toGroupIndex,
      );
  ...
  selectGroupLabel = (newName: string, groupIndex: number): void => {
    if (this.props.appGlobals.currentASForce) {
      this.props.appGlobals.currentASForce.selectGroupLabel(
        newName,
        groupIndex,
      );
      this.props.appGlobals.saveCurrentASForce(
  ...
  removeUnitFromGroup = (asGroupIndex: number, asUnitIndex: number): void => {
    if (this.props.appGlobals.currentASForce) {
      this.props.appGlobals.currentASForce.removeUnitFromGroup(
        asGroupIndex,
        asUnitIndex,
      );
      this.props.appGlobals.saveCurrentASForce(
  ...
  render = (): JSX.Element => {
    ...
    return (
      <TextSection label="Current Force" labelButton={addUnitButton}>
        <p className="text-center">
          <strong>Total Groups</strong>:{" "}
          {this.props.appGlobals.currentASForce.getTotalGroups()}&nbsp;|&nbsp;
          <strong>Total Units</strong>:{" "}
          {this.props.appGlobals.currentASForce.getTotalUnits()}&nbsp;|&nbsp;
          <strong>Total Points</strong>:{" "}
          {this.props.appGlobals.currentASForce.getTotalPoints()}
        </p>

        {this.props.appGlobals.currentASForce.groups.map(
          (asGroup, asGroupIndex) => {
            return (
              <fieldset key={asGroupIndex} className="fieldset">
                <legend>{asGroup.getName(asGroupIndex + 1)}</legend>

                <div className="pull-right">
                  <button
                    onClick={() =>
                      this.props.appGlobals.saveASGroupFavorite(asGroup)
                    }
                    title={
                    ...
                    <label>
                      ...
                      >
                        {unitGroupNames.map((name, nameIndex) => {
                          return <option key={nameIndex}>{name}</option>;
                        })}
                      </select>
                    </label>
                  </div>
                          ...
                          <tbody key={asUnitIndex}>
                            <tr>
                              <td
                                ...
                                className="text-left min-width no-wrap"
                              >
                                {this.props.appGlobals.currentASForce &&
                                this.props.appGlobals.currentASForce.getTotalGroups() >
                                  1 ? (
                                  <div className="drop-down-menu-container">
                                    ...
                                    <ul
                                      className={
                                        this.state.contextMenuGroup ===
                                          asGroupIndex &&
                                        this.state.contextMenuUnit ===
                                          asUnitIndex
                                          ? "styleless dd-menu active"
                                          : "styleless dd-menu"
                                      }
                                    >
                                      <li
                                        onClick={() =>
                                          this.openEditUnit(asUnit)
                                        }
                                      ...
                                      {this.props.appGlobals.currentASForce.groups.map(
                                        (asGroup, asGroupListIndex) => {
                                          return (
                                            <React.Fragment
                                              key={asGroupListIndex}
                                            >
                                              {asGroupListIndex !==
                                              asGroupIndex ? (
                                                <li
                                                  onClick={() =>
                                                    this.moveUnitToGroup(
                                                      asUnitIndex,
                                                      asGroupIndex,
                                                      asGroupListIndex,
                                                    )
                              ...
                              <td>
                                {asUnit.customName ? (
                                  <>
                                    <strong>{asUnit.customName}</strong>
                                    <br />
                                  </>
                                ) : (
                                  <></>
                            ...
                            <tr>
                              <td colSpan={1} className="med-small-text">
                                ...
                                <strong title="This is the Pilot's Skill">
                                  Pilot Skill
                                </strong>
                                : {asUnit.currentSkill}
                                &nbsp;|&nbsp;<strong title="Move">
                                  MV
                                </strong>: {asUnit.currentMove}
                                &nbsp;|&nbsp;
                                <strong title="Armor/Internal Structure values">
                                  A/IS
                                </strong>
                                : {asUnit.armor}/{asUnit.structure}
                                &nbsp;|&nbsp;
                                <strong title="Alpha Strike Damage Bands">
                                  Damage
                                </strong>
                                : {asUnit.damage.short}/{asUnit.damage.medium}/
                                {asUnit.damage.long}
                                {asUnit.overheat && asUnit.overheat > 0 ? (
                                  <>
                                    &nbsp;|&nbsp;
                                    <strong title="Overheat Value">OHV</strong>:{" "}
                                    {asUnit.overheat}
                                  </>
                                ) : null}
                                {asUnit.abilities &&
                                asUnit.abilities.length > 0 ? (
                                  <>
                                    &nbsp;|&nbsp;
                                    <strong title="Special Abilities">
                                      Special
                                    </strong>
                                    : {asUnit.abilities.join(", ")}
                                  </>
                                ) : null}
                              </td>
                              <td className="text-center small-text">
                                {asUnit.mechCreatorUUID ? (
                                  <div title="This mech was created with the 'Mech Builder">
                                    Custom
                                  </div>
                                ) : (
                                  <>&nbsp;</>
                            ...
                            {asUnit
                              .getPilotAbilities()
                              .map((abi: IASPilotAbility | null, abiIndex) => {
                                return (
                  ...
                  <tfoot key="footer">
                    <tr key="groupsum">
                      <td colSpan={2}>
                        ...
                        {asGroup.availableFormationBonuses.length - 1})
                        <select
                          value={
                            asGroup.formationBonus
                              ? asGroup.formationBonus.Name
                              : ""
                          }
                          onChange={(
                            event: React.FormEvent<HTMLSelectElement>,
                          ) => this.updateFormationBonus(event, asGroupIndex)}
                        >
                        ...
                        <br />
                        {asGroup.formationBonus &&
                        asGroup.formationBonus.Name !== "None" ? (
                          <div className="small-pt-text">
                            <strong>Bonus</strong>:{" "}
                            {asGroup.formationBonus.BonusDescription}
                          </div>
                        ) : null}
                      </td>
                      <td className="min-width no-wrap">
                        Points: {asGroup.getTotalPoints()}
                      </td>
                    </tr>
                  </tfoot>
                </table>
