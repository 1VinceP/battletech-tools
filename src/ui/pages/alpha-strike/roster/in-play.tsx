...
export default class AlphaStrikeRosterInPlay extends React.Component<
  IInPlayProps,
  IInPlayState
> {
  ...
  toggleCardMode = (e: React.FormEvent<HTMLSpanElement>): void => {
    if (e && e.preventDefault) e.preventDefault();

    let appSettings = this.props.appGlobals.appSettings;

    appSettings.alphaStrikeInPlayCardMode =
      !appSettings.alphaStrikeInPlayCardMode;
  ...
  closePilotAbility = (e: React.FormEvent<HTMLButtonElement>): void => {
    if (e && e.preventDefault) e.preventDefault();
  ...
  closeSpecialAbility = (e: React.FormEvent<HTMLButtonElement>): void => {
    if (e && e.preventDefault) e.preventDefault();
    ...
    return (
      <>
        {this.state.showPilotAbility ? (
          <StandardModal
            title={
              this.state.showPilotAbility.ability +
              " (" +
              this.state.showPilotAbility.cost +
              ")"
            }
            show={true}
          ...
          >
            <div className="text-center">
              <em>Pilot Special Ability</em> -{" "}
              <span title="Alpha Strike Commander's Edition">
                p{this.state.showPilotAbility.asce_page}
              </span>
            </div>
        ...
        {this.state.showSpecialAbility ? (
          <StandardModal
            title={
              this.state.showSpecialAbility.rawTag +
              ": " +
              this.state.showSpecialAbility.name
            }
            show={true}
          ...
          >
            <div className="text-center">
              <em>
                {getSpecialAbilityTypeName(this.state.showSpecialAbility.type)}
              </em>{" "}
              -{" "}
              <span title="Alpha Strike Commander's Edition">
                p{this.state.showSpecialAbility.asce_page}
              </span>
            </div>
        ...
        <header className="topmenu">
          <ul className="main-menu">
            ...
            {this.props.appGlobals.appSettings.alphaStrikeInPlayCardMode ? (
              <li title="Switch a large list mode">
                <span className="current" onClick={this.toggleCardMode}>
                  <FaList />
                </span>
              </li>
            ) : (
              <li title="Switch to showing 2+ cards per row">
                <span className="current" onClick={this.toggleCardMode}>
                  <FaTh />
                </span>
              </li>
                        ...
                        <>
                          ...
                          >
                            <p>
                              ...
                              {group.formationBonus!.BonusDescription}
                            </p>
                          </div>
                        </>
                      ) : null}
                          ...
                          <React.Fragment key={unitIndex}>
                            <div
                              className={
                                this.props.appGlobals.appSettings
                                  .alphaStrikeInPlayCardMode
                                  ? "col-md-6 col-lg-6 col-xl-3 col-xxl-2"
                                  : "col-md-12"
                              }
                            >
                              <AlphaStrikeUnitSVG
