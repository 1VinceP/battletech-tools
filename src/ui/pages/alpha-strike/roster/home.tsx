...
> {
        ...
        if (data.length === 1) {
          ...
          for (let item of data) {
            if (item.Rules.toLowerCase().trim() === "introductory") {
              ...
              if (
                item.Name &&
                item.Name.toLowerCase().indexOf(" (standard)") > -1
              ) {
                // console.log("Adding intro rules unit via prime in name", item.Name, item.RS)
                let newUnit = new AlphaStrikeUnit();
                newUnit.importMUL(item);
          ...
          if (!foundAUnit) {
            ...
            for (let item of data) {
              if (item.Rules.toLowerCase().trim() === "standard") {
                if (
                  item.Name &&
                  item.Name.toLowerCase().indexOf(" prime") > -1
                ) {
  ...
  openAddingUnits = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
          ...
          <div className="row">
            ...
            <div className="col-lg-6">
              {this.props.appGlobals.favoriteASGroups.length > 0 ? (
                <TextSection label="Favorite Alpha Strike Groups">
                  {this.props.appGlobals.favoriteASGroups.map(
                    (asFavGroup, asFavGroupIndex) => {
                      return (
                        <fieldset key={asFavGroupIndex} className="fieldset">
                          ...
                          <div className="pull-right">
                            <a
                              ...
                              download={
                                "as-group-favorite-export-" +
                                makeURLSlug(asFavGroup.getName(0)) +
                                ".json"
                              }
                            >
                              <FaDownload />
                            ...
                            <button
                              onClick={() =>
                                this.removeFavoriteConfirm(asFavGroupIndex)
                              }
                          ...
                          <div className="text-center">
                            <br />
                            <strong># Units/Points</strong>:{" "}
                            {asFavGroup.getTotalUnits()}/
                            {asFavGroup.getTotalPoints()}
                          </div>
                          ...
                          <table className="table tighter-padding">
                            ...
                            {asFavGroup.members.length > 0 ? (
                              <>
                                {asFavGroup.members.map(
                                  (asFavGroupUnit, asFavGroupUnitIndex) => {
                                    return (
              ...
              <TextSection label="Quickly add a ForcePack">
                {navigator && navigator.onLine ? (
                      ...
                      return (
                        <li key={packindex} className="overflow-hidden">
                          ...
                          <div className="small-text">
                            {joinListWithEndLabel(pack.members, "and")}
                          </div>
                        </li>
                      );
                    })}
                  ...
                  <div className="alert alert-warning">
                    We're sorry, adding a Force Pack needs access to the Master
                    Unit List for unit stats and requires an Internet
                    connection, please connect to the Internet.
                  </div>
                )}
              </TextSection>

              <TextSection label="Import to your AS Favorites">
                <div className="text-small">
                  Use this uploader to restore your favorites from another
                  device. The file will be named, unless it was renamed,
                  "as-group-favorite-export-*,json"
                </div>
