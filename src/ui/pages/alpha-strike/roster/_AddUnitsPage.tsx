...
import {
  AlphaStrikeUnit,
  IASMULUnit,
} from "../../../../classes/alpha-strike-unit";
import { BattleMech } from "../../../../classes/battlemech";
import { getMULASSearchResults } from "../../../../utils";
import {
  getMULAerospaceRoles,
  getMULEraIDs,
  getMULEraLabel,
  getMULGroundRoles,
  getMULTypeIDs,
  getMULTypeLabel,
} from "../../../../utils/mulUtilities";
import { IAppGlobals } from "../../../app-router";
import InputField from "../../../components/form_elements/input_field";
import TextSection from "../../../components/text-section";
import CurrentForceList from "./_CurrentForceList";

export default class AlphaStrikeAddUnitsView extends React.Component<
  IAlphaStrikeAddUnitsViewProps,
  IAlphaStrikeAddUnitsViewState
> {
  constructor(props: IAlphaStrikeAddUnitsViewProps) {
    ...
    this.state = {
      searchResults:
        this.props.appGlobals.appSettings.alphasStrikeCachedSearchResults,
      contextMenuSearch: -1,
  ...
  updateSearchResults = async (): Promise<void> => {
    ...
    data.sort((a: IASMULUnit, b: IASMULUnit): number => {
      ...
      const secondarySort =
        this.state.searchSort === "Name" ? "BFPointValue" : "Name";
            ...
            <TextSection label="Search for Units">
              ...
              {navigator && navigator.onLine ? (
                      ...
                      {this.state.searchResults.length > 0 ? (
                        <>
                          {this.state.searchResults.map(
                            (asUnit: IASMULUnit, unitIndex: number) => {
                              return (
                                <tbody key={unitIndex}>
                                  <tr>
                                    <td
                                  ...
                                  <tr>
                                    <td colSpan={4} className=" text-left">
                                      <strong title="Move">MV</strong>:{" "}
                                      {asUnit.BFMove}
                                      &nbsp;|&nbsp;
                                      <strong title="Armor/Internal Structure values">
                                        A/IS
                                      </strong>
                                      : {asUnit.BFArmor}/{asUnit.BFStructure}
                                      &nbsp;|&nbsp;
                                      <strong title="Alpha Strike Damage Bands">
                                        Damage
                                      </strong>
                                      : {asUnit.BFDamageShort}/
                                      {asUnit.BFDamageMedium}/
                                      {asUnit.BFDamageLong}
                                      {asUnit.BFOverheat &&
                                      asUnit.BFOverheat > 0 ? (
                                        <>
                                          &nbsp;|&nbsp;
                                          <strong title="Overheat Value">
                                            OHV
                                          </strong>
                                          : {asUnit.BFOverheat}
                                        </>
                        ...
                        <>
                          {this.props.appGlobals.appSettings
                            .alphaStrikeSearchTerm.length < 3 ? (
                            <tbody>
                              <tr>
                                <td className="text-center" colSpan={7}>
                                  Please type a search term 3 or more
                                  characters.
                                </td>
                              </tr>
                            </tbody>
                            ...
                            <tbody>
                              <tr>
                                <td className="text-center" colSpan={7}>
                                  Sorry, there are no matches with those
                                  parameters. It is a remote possibility that
                                  the MUL is down if other searches don't work.
                                </td>
                              </tr>
                            </tbody>
                          )}
                ...
                <div className="alert alert-warning">
                  We're sorry, searching the Master Unit List for units requires
                  an Internet connection, please connect to the Internet.
                </div>
              )}
            </TextSection>

            {this.props.appGlobals.battleMechSaves &&
            this.props.appGlobals.battleMechSaves.length > 0 ? (
              <TextSection label="Your Created BattleMechs">
                <table className="table">
                  <thead>
                    <tr>
                      <th>&nbsp;</th>
                  ...
                  {this.props.appGlobals.battleMechSaves.map(
                    (bm, unitIndex) => {
                      ...
                      return (
                        <tbody key={unitIndex}>
                          <tr>
                              ...
                              this.props.appGlobals.currentASForce.getTotalGroups() >
                                ...
                                <div className="drop-down-menu-container">
                                  <button
                                    className="btn btn-primary btn-sm"
                                    onClick={() =>
                                      this.toggleContextMenuSavedBattleMechs(
                                        unitIndex,
                                      )
                                    }
                                    title="Open the context menu for this unit"
                                  ...
                                  <ul
                                    className={
                                      this.state.contextMenuSavedBattleMechs ===
                                      unitIndex
                                        ? "styleless dd-menu active"
                                        : "styleless dd-menu"
                                    }
                                      ...
                                      (asGroup, asGroupIndex) => {
                                        return (
                                          ...
                                          >
                                            <FaPlus />
                                            &nbsp; Add to{" "}
                                            {asGroup.getName(asGroupIndex + 1)}
                                          </li>
                                        );
                                      },
                                    )}
                            ...
                            <td title={"UUID: " + asUnit.mechCreatorUUID}>
                              {asUnit.name}
                            </td>
