...
export default class BattleMechTableGroup extends React.Component<
  IBattleMechTableGroupProps,
  IBattleMechTableGroupState
> {
  ...
  addViewedUnit = (e: React.FormEvent<HTMLButtonElement> | null) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    if (this.props.appGlobals.currentCBTForce && this.state.viewingUnit) {
      let bmGroup =
        this.props.appGlobals.currentCBTForce.groups[this.props.bmGroupIndex];
      let newBM = new BattleMech(this.state.viewingUnit.exportJSON());
      newBM.newUUID();
      bmGroup.members.push(newBM);
  ...
  closeAddingUnits = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  onAddUnitsChange = (nv: BattleMechGroup): void => {
    if (
      ...
      this.props.appGlobals.currentCBTForce.groups[
        this.state.addingUnitsGroupIndex
      ]
    ) {
  ...
  openEditDialog = (
    ...
    if (this.props.appGlobals.currentCBTForce) {
      let currentCBTForce = this.props.appGlobals.currentCBTForce;
      let editUnit = new BattleMech(
        currentCBTForce.groups[this.props.bmGroupIndex].members[
          editUnitIndex
        ].exportJSON(),
      );
      this.setState({
  ...
  saveUnit = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  removeUnit = (
    ...
    this.props.appGlobals.openConfirmDialog(
      ...
      () => {
        if (this.props.appGlobals.currentCBTForce) {
          let currentCBTForce = this.props.appGlobals.currentCBTForce;
          currentCBTForce.groups[this.props.bmGroupIndex].members.splice(
            removeUnitIndex,
            1,
          );
          this.props.appGlobals.saveCurrentCBTForce(currentCBTForce);
  ...
  closeEditDialog = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
      ...
      <>
        {this.state.editingUnit ? (
          ...
          >
            <div className="small-text">
              Here you may edit the Mech Nickname, Mechwarrior name, and skills.
              Skill changes Will display both base amd adjusted BV2.
            </div>
            <InputField
              label="'Mech Nickname"
            ...
            <div className="row">
              ...
              <div className="col-md-6">
                <fieldset className="fieldset">
                  ...
                  <div>
                    <strong>Base BV2</strong>:{" "}
                    {this.state.editingUnit.getBattleValue()}
                  </div>
                  <div>
                    <strong>Adjusted BV2</strong>:{" "}
                    {this.state.editingUnit.getPilotAdjustedBattleValue()}
                  </div>
                </fieldset>
              </div>
            </div>
            ...
            title={
              ...
              this.props.appGlobals.currentCBTForce.groups[
                ...
                ? "Adding units to '" +
                  this.props.appGlobals.currentCBTForce.groups[
                    this.state.addingUnitsGroupIndex
                  ].getName(this.state.addingUnitsGroupIndex) +
                  "'"
                : "Adding units to Current Force"
            }
          >
        ...
        {this.state.viewingUnit ? (
          ...
          <table className="table">
                  ...
                  <th className="min-width no-wrap">
                    {this.props.showAdd ? (
                      <button
                        ...
                        onClick={(e) =>
                          this.openAddingUnits(e, this.props.bmGroupIndex)
                        }
                      >
                        Add
                      </button>
            ...
            {this.props.appGlobals.currentCBTForce.groups[
              this.props.bmGroupIndex
            ].members.length > 0 ? (
              <>
                {this.props.appGlobals.currentCBTForce.groups[
                  this.props.bmGroupIndex
                ].members.map((mechObj, mechIndex) => {
                      ...
                      <tr>
                        {this.props.showEdit ? (
                          <td className="min-width no-wrap text-center">
                            ...
                            <button
                              onClick={(e) =>
                                this.removeUnit(e, mechObj, mechIndex)
                              }
                      ...
                      <tr>
                        <td colSpan={7}>
                          {mechObj.isUnderStrength() ? (
                            <div className="text-center color-bright-red">
                              <strong>This Mech is Damaged</strong>
                            </div>
                          ) : (
                            <div className="text-center">
                              This Mech is Undamaged
                            </div>
                          )}
                          <div className="bars">
                            <StatBar
                            ...
                            <StatBar
                              ...
                              height={8}
                              title={
                                "Current Heat Status: " + mechObj.currentHeat
                              }
                            />
                          </div>
                        </td>
              ...
              <tbody>
                <tr>
                  <td
                    colSpan={this.props.showEdit ? 7 : 6}
            ...
            <tfoot>
              <tr>
                ...
                <td
                  colSpan={this.props.showEdit ? 4 : 3}
                  className="text-center"
                >
                  {this.props.appGlobals.currentCBTForce.groups[
                    this.props.bmGroupIndex
