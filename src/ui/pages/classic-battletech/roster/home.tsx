...
export default class BattleMechRosterHome extends React.Component<
  IHomeProps,
  IHomeState
> {
  ...
  removeGroup = (bmGroupIndex: number): void => {
    if (
      this.props.appGlobals.currentCBTForce &&
      this.props.appGlobals.currentCBTForce.groups.length > bmGroupIndex
    ) {
      if (
        this.props.appGlobals.currentCBTForce.groups[
          bmGroupIndex
        ].getTotalUnits() === 0
      ) {
  ...
  selectGroupLabel = (newName: string, groupIndex: number): void => {
    if (this.props.appGlobals.currentCBTForce) {
      this.props.appGlobals.currentCBTForce.selectGroupLabel(
        newName,
        groupIndex,
      );
      this.props.appGlobals.saveCurrentCBTForce(
...
/> */}
        ...
        <UIPage
          current="classic-battletech-roster"
          appGlobals={this.props.appGlobals}
        >
          <div className="alert alert-danger">
            <h4 className="text-center">In Development</h4>
            <p>
              This area will be the equivalent of the Roster Maker in Alpha
              Strike. If anything seems to work, it likely doesn't. Use at your
              owm risk!
            </p>
            <p>
              <strong>Update 2022 Apr 28</strong>: The Print Force area should
              be working great. The Play Mode is making huge srides and getting
              closer! Initial tests are working for all phases, but it's still
              heavily under testing. I've not got to the Mechwarrior hits yet.
              There are quite a few interface bugs which are bothering me.{" "}
            </p>
            <del>
              <p>
                <strong>Update 2022 Apr 3</strong>: With the exception of the
                Play/Print buttons this <em>MIGHT</em> work. I'll work on the
                Print button first, then onward to the intense Play button
                (since I'll have to code the electronic record sheet functions)
              </p>
            </del>
            <p className="text-center">
              ...
              <a
                target="github"
                href="https://github.com/jdgwf/battletech-tools/issues"
              >
                Issue
              </a>{" "}
              for anything here - I'll likely already be aware of it, but if
              you've got an idea for play-flow let me know. I've tried to make
              it so that everything is just a step-by-step process as you roll
              the dice and move your models.
            </p>
          </div>
          {this.props.appGlobals.currentCBTForce &&
          this.props.appGlobals.currentCBTForce.getTotalUnits() > 0 ? (
            <div className="row">
              <div className="col-6">
                <Link
          ...
          <div className="row">
            <div className="col-lg-6">
              <TextSection label="Current CBT Force">
                {this.props.appGlobals.currentCBTForce ? (
                  <p className="text-center">
                    <strong>Total Groups</strong>:{" "}
                    {this.props.appGlobals.currentCBTForce.getTotalGroups()}
                    &nbsp;|&nbsp;
                    <strong>Total Units</strong>:{" "}
                    {this.props.appGlobals.currentCBTForce.getTotalUnits()}
                    &nbsp;|&nbsp;
                    <strong>Total Tons</strong>:{" "}
                    {this.props.appGlobals.currentCBTForce.getTotalTons()}
                    &nbsp;|&nbsp;
                    <strong>Total BV2</strong>:{" "}
                    {this.props.appGlobals.currentCBTForce.getTotalBV2()}
                  </p>
                ) : null}
                {this.props.appGlobals.currentCBTForce ? (
                  <>
                    {this.props.appGlobals.currentCBTForce.groups.map(
                      (bmGroup, bmGroupIndex) => {
                        return (
                          <fieldset key={bmGroupIndex} className="fieldset">
                            <legend>
                              {bmGroup.groupLabel} #{bmGroupIndex + 1}
                            </legend>

                            <div className="pull-right">
                              <button
                                onClick={() =>
                                  this.props.appGlobals.saveCBTGroupFavorite(
                                    bmGroup,
                                  )
                                }
                                title={
                                  bmGroup.members.length === 0
                                    ? "A group need to have members to save as a favorite"
                                    : "Click here to add this group to your favorites."
                                }
                            ...
                            <div className="width-80">
                              ...
                              <div className="width-50">
                                <InputField
                                  ...
                                  placeholder={
                                    "Custom " + bmGroup.groupLabel + " Name"
                                  }
                                  onChange={(
            ...
            <div className="col-lg-6">
              {this.props.appGlobals.favoriteCBTGroups.length > 0 ? (
                <TextSection label="Favorite CBT Groups">
                  {this.props.appGlobals.favoriteCBTGroups.map(
                    (favGroup, favGroupIndex) => {
                      return (
                        <fieldset className="fieldset" key={favGroupIndex}>
                          <legend>
                            {favGroup.getName(favGroupIndex, true)}
                          </legend>
                          ...
                          <div className="pull-right">
                            <a
                              ...
                              download={
                                "cbt-group-favorite-export-" +
                                makeURLSlug(favGroup.getName(0)) +
                                ".json"
                              }
                            >
                              <FaDownload />
                            ...
                            <button
                              onClick={() =>
                                this.removeFavoriteConfirm(favGroupIndex)
                              }
                          ...
                          <table className="table">
                            <thead>
                              <tr>
                                ...
                                <th>Tons</th>
                                <th>Tech</th>
                                <th className="min-width no-wrap text-center">
                                  Piloting
                                </th>
                                <th className="min-width no-wrap text-center">
                                  Gunnery
                            ...
                            {favGroup.members.map((mechObj, mechIndex) => {
                              let pilotBV2 =
                                mechObj.getPilotAdjustedBattleValue();
                              ...
                              return (
                                <tbody key={mechIndex}>
                                  <tr>
                                    ...
                                    <td className="min-width no-wrap text-center small-text">
                                      {mechObj.getTech().name}
                                    </td>
                                    <td className="min-width no-wrap text-center">
                                      {mechObj.pilot.piloting}
                                    </td>
                                    <td className="min-width no-wrap text-center">
                                      {mechObj.pilot.gunnery}
                                    </td>
                                    <td className="min-width no-wrap text-center">
                                      {pilotBV2 !== baseBV2 ? (
                                        <>
                            ...
                            <tfoot>
                              <tr>
                                ...
                                <td colSpan={1} className="no-wrap text-right">
                                  BV2: {favGroup.getTotaBV2()}
                                </td>
                              </tr>
                            </tfoot>
                          </table>
              ...
              <TextSection label="Import to your CBT Favorites">
                <div className="text-small">
                  Use this uploader to restore your favorites from another
                  device. The file will be named, unless it was renamed,
                  "cbt-group-favorite-export-*,json"
                </div>

                <label title="Click here to select a JSON file exported this page">
                  Import JSON:&nbsp;
                  <input
                    type="file"
                    style={{ width: "auto" }}
