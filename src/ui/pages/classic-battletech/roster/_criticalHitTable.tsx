...
export default class InPlayCriticalHitTable extends React.Component<
  IInPlayCriticalHitTableProps,
  IInPlayCriticalHitTableState
> {
  toggleCritical = (location: string, critSlotIndex: number) => {
  ...
  render = (): JSX.Element => {
    ...
    return (
      <fieldset className="fieldset">
        <legend>
          {getLocationName(this.props.location, this.props.mechData.isQuad())}
        </legend>
        <table className="full-width">
          <tbody>
            {makeRange(0, this.props.numberCritSlots - 1).map(
              (critSlotIndex) => {
                ...
                if (critSlotIndex === 6) {
                  firstColData = <td rowSpan={6}>4-6</td>;
                }
                if (critSlotIndex === 0) {
                  firstColData = <td rowSpan={6}>1-3</td>;
                }
                  ...
                  return (
                    <React.Fragment key={critSlotIndex}>
                      <tr>
                        ...
                        <td
                          className={
                            this.props.mechData.isCriticalDamaged(
                              this.props.location,
                              critSlotIndex,
                            )
                              ? "color-brighter-red"
                        ...
                        <td
                          className={
                            this.props.mechData.isCriticalDamaged(
                              ...
                              ? "color-brighter-red cursor-pointer font-weight-700"
                              : "cursor-pointer"
                          }
                          onClick={(e) =>
                            this.toggleCritical(
