...
export default class MechCreatorImports extends React.Component<
  IHomeProps,
  IHomeState
> {
  ...
  updateMechNameFilter = (e: React.FormEvent<HTMLInputElement>) => {
  ...
  updateMechRulesFilter = (e: React.FormEvent<HTMLSelectElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    let appSettings = this.props.appGlobals.appSettings;
  ...
  _filterSSWMechs = (a: BattleMech): boolean => {
    if (
      ...
      a
        ...
        .toLowerCase()
        .indexOf(
          this.props.appGlobals.appSettings.mechNameFilter.trim().toLowerCase(),
        ) === -1
    ) {
      return false;
    }
    ...
    if (
      ...
      a.basicSSWInfo?.rules_level_ssw !==
  ...
  importUnit = (e: React.FormEvent<HTMLButtonElement>, unit: BattleMech) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    if (unit) {
  ...
  closeTROModal = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  importTRO = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  openTROModal = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  viewUnit = (e: React.FormEvent<HTMLButtonElement>, bm: BattleMech) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  closeViewUnit = (e: React.FormEvent<HTMLButtonElement> | null) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    this.setState({
  ...
  importViewedUnit = (e: React.FormEvent<HTMLButtonElement> | null) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  render = (): JSX.Element => {
    if (!this.props.appGlobals.currentBattleMech) return <></>;
    return (
      <UIPage
        current="classic-battletech-mech-creator"
      ...
      >
          ...
          {this.state.viewingUnit ? (
            <>
              <h3 className="text-center">
                Viewing {this.state.viewingUnit.model}
              </h3>
              <div className="row">
                <div className="col">
                  <SanitizedHTML
                ...
                <div className="col">
                  <BattleMechSVG mechData={this.state.viewingUnit} />
                </div>
              </div>
            </>
          ) : null}
        ...
        >
          <div className="row">
            <div className="col">
              ...
              <p>
                Because of formatting of copy/paste in Apple Preview, etc, it
                seems that Adobe Reader is required for ease of use. You could
                use another, but the copy/paste of Preview and other PDF viewer
                apps can be erratic with page formatting.
              </p>
            </div>
            ...
            <div className="col">
              ...
              <p>
                What will never work: The standard PDF TROs don't let you know
                what actuators are missing and where to set the Heat Sink
                criticals, so you'll have to edit that yourself. That said, this
          ...
          <div className="row">
            ...
            <div className="col">
              {this.state.ParsedTRO ? (
                <SanitizedHTML
                  html={this.state.ParsedTRO.makeTROHTML()}
                  raw={true}
                />
              ) : (
                <div className="text-center">
                  ...
                  <p>
                    Remember: It seems that only Adobe Acrobat does a good job
                    at copying all the text in an orderly fashion.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="row">
          ...
          <div className="col-md-9 col-lg-6">
            <TextSection label="Imports From Skunkwerks">
              <div className="alert alert-info">
                <p>
                  This is now working perfectly in a limited fashion! Be sure to
                  only semi-trust the units with a{" "}
                  <FaCheckCircle
                    title="This import looks good to add!"
                ...
                <p>
                  Since we're still short on base equipment for Clans, right now
                  only the TRO3039 is loadable. Don't worry we'll get it taken
                  care of as this app develops. TRO 3050/3055 is the next
                  target!
                </p>
              ...
              <table className="table">
                  ...
                  <tbody>
                    <tr>
                      <td className="text-center" colSpan={5}>
                        <br />
                        No 'mechs found for import. This should not be!
                        <br />
                        <br />
                      </td>
                    </tr>
                ...
                {this.props.appGlobals.sswMechObjects.length > 0 &&
                this.props.appGlobals.sswMechObjects.filter(
                  this._filterSSWMechs,
                ).length === 0 ? (
                  <tbody>
                    <tr>
                      <td className="text-center" colSpan={5}>
                        <br />
                        There are no 'mechs found with your filter preferences
                        <br />
                        <br />
                      </td>
                    </tr>
                  ...
                  <>
                    {this.props.appGlobals.sswMechObjects
                      .sort(sortByMechName)
                      .filter(this._filterSSWMechs)
                      .map((bmObj, bmIndex) => {
                        let perfectImport = true;
                        let problems: string[] = [];
                        if (
                          bmObj.basicSSWInfo &&
                          bmObj.basicSSWInfo.bv2 !== bmObj.getBattleValue()
                        ) {
                          perfectImport = false;
                          problems.push(
                            "BV2 doesn't match! SSW " +
                              bmObj.basicSSWInfo.bv2 +
                              " vs calculated " +
                              bmObj.getBattleValue(),
                          );
                        }
                        if (
                          bmObj.basicSSWInfo &&
                          bmObj.basicSSWInfo.cbill_cost !==
                            bmObj.getCBillCostNumeric(true)
                        ) {
                          perfectImport = false;
                          problems.push(
                            "Cbill Cost doesn't match! SSW " +
                              bmObj.basicSSWInfo.cbill_cost +
                              " vs calculated " +
                              bmObj.getCBillCost(true),
                          );
                        }
                        if (bmObj.sswImportErrors.length > 0) {
                          perfectImport = false;
                          problems.push(
                            bmObj.sswImportErrors.length + " import errors!",
                          );
                        }
                            ...
                            <tr>
                              <td>
                                ...
                                {perfectImport ? (
                                  ...
                                  <FaTimesCircle
                                    title={
                                      "Not an accurate import, not recommended for adding to your force. " +
                                      problems.join("; ")
                                    }
                                    className="color-red"
                                  />
                              ...
                              <td className="min-width no-wrap text-left">
                                {bmObj.getTech().name}
                              </td>
                              <td className="min-width no-wrap text-left">
                                {getSSWRulesLevelLabel(
                                  bmObj.basicSSWInfo?.rules_level_ssw
                                    ? bmObj.basicSSWInfo?.rules_level_ssw
                                    : 0,
                                  true,
                                )}
              ...
              See the <Link to="ssw-sanity-check">Sanity Test</Link> page for
              compatibility
              <div className="clear-both overflow-hidden">
                ...
                <div className="text-left inline-block">
                  ...
                  >
                    <FaArrowCircleLeft /> Exports
                  </Link>
                </div>
              </div>
            </TextSection>

            <TextSection label="Import TRO Text">
          ...
          <div className="d-none d-lg-block col-lg-4">
            <TextSection>
              <div className="mech-tro">
                <SanitizedHTML
                  raw={true}
                  html={this.props.appGlobals.currentBattleMech.makeTROHTML()}
                />
              </div>
            </TextSection>
          </div>
