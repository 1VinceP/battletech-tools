...
> {
  ...
  constructor(props: IEquipmentEditorProps) {
    ...
    if (!this.fileDataList[currentList]) {
      currentListData = JSON.parse(
        JSON.stringify(
          this.fileDataList["mech-is-equipment-weapons-ballistic"],
        ),
      );
    } else {
      currentListData = JSON.parse(
        JSON.stringify(this.fileDataList[currentList]),
      );
    }
  ...
  showJSON = (e: React.FormEvent<HTMLButtonElement>) => {
  ...
  closeJSON = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    this.setState({
  ...
  selectList = (e: React.FormEvent<HTMLSelectElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
      ...
      if (this.state.isDirty) {
        this.props.appGlobals.openConfirmDialog(
          ...
          () => {
            ...
            currentListData = JSON.parse(
              JSON.stringify(this.fileDataList[e.currentTarget.value]),
            );
...
`;
    ...
    rv +=
      "export const " +
      this._getFileVariable(this.state.currentList) +
      ": IEquipmentItem[] = ";

    rv += exportCleanJSON(this.state.currentListData);

    return rv;
  };

  closeEditItem = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    this.setState({
  ...
  _getCategoryName = (): string => {
    if (this.state.currentList.toLocaleLowerCase().indexOf("-energy") > -1) {
      ...
      this.state.currentList.toLocaleLowerCase().indexOf("-ballistic") > -1
    ) {
      return "Ballistic Weapons";
    } else if (
      this.state.currentList.toLocaleLowerCase().indexOf("-misc") > -1
    ) {
      return "Miscellaneous Equipment";
    } else if (
      this.state.currentList.toLocaleLowerCase().indexOf("-missiles") > -1
    ) {
  ...
  _getTechnologyName = (): string => {
    return this.state.currentList.indexOf("-is-") > -1
      ? "Inner Sphere"
      : "Clan";
  };
  ...
  _sortByTag = (a: IEquipmentItem, b: IEquipmentItem): number => {
    if (a.tag.toLocaleLowerCase().trim() > b.tag.toLocaleLowerCase().trim()) {
      return 1;
    } else if (
      a.tag.toLocaleLowerCase().trim() < b.tag.toLocaleLowerCase().trim()
    ) {
      return -1;
    } else {
      return 0;
      ...
      <UIPage current="equipment-editor" appGlobals={this.props.appGlobals}>
        <div className="alert alert-warning text-center">
          <p className="no-margins">
            <strong>Developers only!</strong> This area is getting{" "}
            <em>really close</em> for data-entry, but this should never be used
            by the average user.
          </p>
        </div>
        {this.state.showJSON ? (
          ...
          >
            <div className="alert alert-info">
              <strong>Remember</strong>: editing this page won't make the
              changes permanent. You'll need top copy/paste the contents below
              into /src/data/{this.state.currentList + ".ts"}
            </div>
            <textarea
              style={{
                width: "100%",
        ...
        {this.state.editItem ? (
          <StandardModal
            ...
            onSaveAsNew={
              this.state.editItemIndex > -1 ? this.saveItemAsNew : undefined
            }
            title={
              this.state.editItemIndex > -1 ? "Editing Item" : "Adding Item"
            }
          >
            <EquipmentEditForm
        ...
        <label>
          ...
          >
            {Object.keys(this.fileDataList).map((fileName, fileIndex) => {
              return (
                <option key={fileIndex} value={fileName}>
                  {fileName}.ts
                </option>
              );
            })}
        ...
        <table className="table">
          <thead>
            ...
            <tr>
              ...
              <th className="min-width text-right no-wrap">
                <button
          ...
          {this.state.currentListData
            .sort(sortEquipment)
            .map((item: IEquipmentItem, itemIndex: number) => {
              return (
                <tbody key={itemIndex}>
                  <tr>
                    <td>
                        ...
                        <>
                          {item.damageDivisor && item.damageDivisor > 0 ? (
                            ...
                            <>
                              {item.damageClusters ? (
                                <>
                                  Clusters: {item.damageClusters}
                                  <br />
                                  Damage Per: {item.damagePerCluster}
                                  <br />
                                </>
                              ) : (
                                <>
                      ...
                      {item.isAmmo ? (
                        ...
                        <>
                          {item.isMelee ? (
                            ...
                            <>
                              {item.range.short} / {item.range.medium} /{" "}
                              {item.range.long}
                              &nbsp;
                              <span title="Aero range">
                                (
                                {item.rangeAero ? (
                                  getAeroRangeLabel(item.rangeAero)
                                ) : (
                                  <span className="color-red">N/E</span>
                                )}
                                )
                              </span>
                            </>
                    ...
                    <td className="text-center no-wrap">
                      {item.criticalsDivisor && item.criticalsDivisor > 1 ? (
                        <>
                          Mass ÷ {item.criticalsDivisor}
                          <br />
                        </>
                      ) : (
                        <>
                          {item.space.battlemech}
                          <br />
                        </>
                      )}
                    </td>
                    ...
                    <td className="text-center no-wrap ">
                      {item.heat}
                      {item.heatPerShot ? <>/shot</> : null}
                      <br />
                    </td>
                    <td className="text-center no-wrap ">
                      {item.battleValue}
                      {item.battleValueDefensive ? (
                        <sup title="Battle Value is Calculated as Defensive">
                          D
                        </sup>
                      ) : null}
                      <br />
                    ...
                    <td className="text-center no-wrap ">
                      {addCommas(item.cbills)}
                      <br />
                    </td>

                    <td className="text-center no-wrap ">
                      <div className="text-center explosive-tag">
                        {item.explosive ? <MineExplosion /> : null}
                      </div>
                    </td>
                    <td className="text-center no-wrap ">
                      {item.introduced}-
                      {item.extinct > 0 ? item.extinct : "current"}
                      <br />
                      {item.reintroduced > 0 ? (
