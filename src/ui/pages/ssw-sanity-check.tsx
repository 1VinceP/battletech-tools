...
export default class SSWSanityCheck extends React.Component<
  ISSWSanityCheckProps,
  ISSWSanityCheckState
> {
  needsASValue: string[] = [];
  ...
  clearMULCache = (e: React.FormEvent<HTMLButtonElement>) => {
  ...
  fetchMULData = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  updateSSWRulesFilter = (e: React.FormEvent<HTMLSelectElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
  ...
  _getAlphaStrikeValues = async (): Promise<void> => {
    ...
    for (
      let mechIndex = this.needsASValue.length - 1;
      mechIndex >= 0;
      mechIndex--
    ) {
      ...
      if (data.length === 1) {
        ...
        for (let mech of data) {
          if (
            mech.Variant &&
            mech.Variant.toLowerCase().trim() === mechName.toLowerCase().trim()
          ) {
            appSettings.asValues[mechName] = data[0].BFPointValue;
            this.needsASValue.splice(mechIndex, 1);
            break;
  ...
  closeHTML = (e: React.FormEvent<HTMLButtonElement>) => {
    if (e && e.preventDefault) {
      e.preventDefault();
    }
    ...
    return (
      <UIPage current="ssw-sanity-check" appGlobals={this.props.appGlobals}>
        ...
        <div className="alert alert-info">
          <p>
            This is an internal testing page for all the Solaris Skunk Werks
            imported data. This imports each, then compares the calculated
            C-Bill and BV2 values against the stated values in the SSW XML file.
            If there's a discrepancy then it'll notify, and allow for a
            side-by-side comparison when checking against the SSW app.
          </p>
          <p>
            At the bottom of the table, there'll be a total and percentage
            accuracy.
          </p>
        </div>
        <div className="row">
        ...
        <table className="table">
            ...
            <tr>
              ...
              <th>
                ...
                <div className="small-text">
                  You'll have to Fetch MUL data to cache
                </div>
              </th>
              <th>Import Errors</th>
            </tr>
            ...
            .map((tempMech, mechIndex) => {
              ...
              if (sswData) {
                ...
                if (tempMech) {
                  ...
                  if (
                    typeof this.props.appGlobals.appSettings.asValues[
                      tempMech.model
                    ] !== "undefined" ||
                    this.props.appGlobals.appSettings.asValues[
                      tempMech.getName()
                    ] === -1
                  ) {
                    MULASValue =
                    ...
                    if (MULASValue === -1) {
                      if (this.needsASValue.indexOf(tempMech.model) === -1) {
                        console.log(
                          "Adding to needsASValue due to -1 value",
                          tempMech.model,
                        );
                        // IS only using .model instead of .getName() - for some reason MUL API is removing or ignoring the space - even converting space to %20
                      ...
                      <tr>
                        ...
                        <td>
                          {SSWCbills !== tempMechCBills ? (
                            <div>
                              ...
                              &nbsp;CBill Costs doesn't match:
                              <br />
                              SSW: {addCommas(SSWCbills)} != JBT:{" "}
                              {addCommas(tempMechCBills)}
                              {Math.abs(SSWCbills - tempMechCBills) < 3 ? (
                                <div className="color-green">
                                  ..but it's close - rounding error?
                                </div>
                              ) : null}
                              <button
                        ...
                        <td>
                          {SSWBV2 !== tempMechBV2 ? (
                            <div>
                              <FaTimesCircle className="color-red" />
                              &nbsp;BV2 doesn't match: SSW: {addCommas(
                                SSWBV2,
                              )}{" "}
                              != JBT: {addCommas(tempMechBV2)}
                              <button
                                className="btn btn-primary full-width btn-xs"
                                onClick={(e) =>
                                  this.viewHTML(
                            ...
                            <div>
                              <FaCheckCircle className="color-green" />
                              &nbsp;BV2 matches: {addCommas(tempMechBV2)}
                            </div>
                          )}
                        </td>
                        <td>
                          {MULASValue === 0 ? (
                            <div>?? Couldn't obtain MUL Value</div>
                          ) : (
                            <>
                              {MULASValue !== tempMechBFValue ? (
                                <div>
                                  <FaTimesCircle className="color-red" />
                                  &nbsp;PV doesn't match: MUL:{" "}
                                  {addCommas(MULASValue)} != JBT:{" "}
                                  {addCommas(tempMechBFValue)}
                                  <button
                                    className="btn btn-primary full-width btn-xs"
                                    onClick={(e) =>
                                      this.viewHTML(
                                ...
                                <div>
                                  <FaCheckCircle className="color-green" />
                                  &nbsp;PV matches: {addCommas(tempMechBFValue)}
                                </div>
                              )}
                            </>
                          )}
          ...
          <tfoot>
            <tr>
              <th>Total Mechs Here: {totalMechs}</th>
              <th></th>
              <th></th>
              <th></th>
              <th>
                CBill Errors: {cbillDiscrpancies} (
                {Math.round((cbillDiscrpancies / totalMechs) * 10000) / 100} %)
              </th>
              <th>
                BV2 Errors: {bv2Discrpancies} (
                {Math.round((bv2Discrpancies / totalMechs) * 10000) / 100} %)
              </th>
              <th>
                MUL AS Value Errors: {bfValueDiscrpancies} (
                {Math.round((bfValueDiscrpancies / totalMechs) * 10000) / 100}{" "}
                %)
              </th>
              <th>
                Import Errors: {importErrors} (
                {Math.round((importErrors / totalMechs) * 10000) / 100} %)
              </th>
            </tr>
          </tfoot>
        </table>
